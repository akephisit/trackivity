...
use crate::middleware::session::{
    SessionState, AdminUser, SuperAdminUser, FacultyAdminUser
};
...
pub async fn get_admin_sessions_overview(
    ...
    let faculty_filter = params.get("faculty_id")
        .and_then(|f| f.parse::<Uuid>().ok());
    
    let admin_level_filter = params.get("admin_level");
    
    // Get all active sessions
    let session_ids = session_state.redis_store.get_active_sessions(Some(limit * 2)).await
    ...
    for session_id in &session_ids {
        if let Ok(Some(session)) = session_state.redis_store.get_session(session_id).await {
            ...
            if let Ok(Some(enhanced_info)) = get_enhanced_session_info(
                &session_state, 
                &admin.session_user.session_id,
                &session
            ).await {
                // Apply filters
                ...
                if let Some(level_filter) = admin_level_filter {
                    let session_level = enhanced_info.admin_level
                        .as_ref()
                        .map(|l| format!("{:?}", l))
                ...
                if let Some(faculty_name) = &enhanced_info.faculty_name {
                    ...
                    *faculty_breakdown.entry("No Faculty".to_string()).or_insert(0) += 1;
                }
                
                let level_name = enhanced_info.admin_level
                    .as_ref()
                    .map(|l| format!("{:?}", l))
                    .unwrap_or_else(|| "Student".to_string());
...
pub async fn get_session_stats(
    ...
    let mut user_session_counts: HashMap<Uuid, (User, AdminRole, usize, DateTime<Utc>)> = HashMap::new();
            ...
            if let Ok(Some(user)) = get_user_by_id(&session_state, session.user_id).await {
                ...
                *sessions_by_level.entry(level_name).or_insert(0) += 1;
                ...
                if let Some(ref role) = admin_role {
                    ...
                    *sessions_by_faculty.entry("No Faculty".to_string()).or_insert(0) += 1;
                }
                
                // Count by device type
                let device_type = session.device_info
                    .get("device_type")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown");
                *sessions_by_device.entry(device_type.to_string()).or_insert(0) += 1;
    ...
    let top_active_users = user_counts
        .into_iter()
        .map(|(user_id, (user, admin_role, count, last_activity))| TopActiveUser {
            user_id,
...
pub async fn bulk_session_action(
    ...
    match req.action.as_str() {
        "revoke" => {
            let reason = req.reason.unwrap_or_else(|| format!("Bulk revocation by {}", admin.session_user.email));
        ...
        "extend" => {
            ...
            for session_id in &req.session_ids {
                match session_state.redis_store.extend_session(session_id, new_expiry).await {
                    Ok(true) => {
                        processed_count += 1;
                        // Log the action
                        let _ = log_session_action(
...
async fn get_enhanced_session_info(
    ...
    let (faculty_name, department_name) = if let Some(department_id) = user.department_id {
        ...
        let faculty_name = if let Some(dept) = &department {
            get_faculty_by_id(session_state, dept.faculty_id).await?
                .map(|f| f.name)
        } else {
            None
        };
